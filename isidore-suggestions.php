 ...
 * Description:	With Isidore Suggestions, you can expand your posts with recommendations from Isidore, the research platform in humanities with more than 4 millions datasets.
 * Version:		2.3.0
 * Author:		HUMA-NUM
...
class Isidore_suggestions extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( 'on' == $instance['isidore-input-search'] ) {
			...
			$widget_content .= '<form id="isidore-search-form-' . $this->number . '" role="search" method="get" class="search-form" action="//www.rechercheisidore.fr/search" autocomplete="off">
				<label>
			...
			if ( $my_theme_name == 'Twenty Sixteen' or $my_theme_template == 'twentysixteen' ) {
	...
	function isidore_query_builder( $tags_array, $disciplines_array, $fts_array ) {
		$query				= 'http://api.rechercheisidore.fr/repository/search?afs:query=' .
                                '&afs:log=ISI_SUGGESTIONS_WP' .
                                '&afs:facetDefault=replies=0' .
                                '&afs:ip=' . urlencode( $this->isidore_get_user_ip() ) .
                                '&afs:userAgent=' . urlencode( $_SERVER['HTTP_USER_AGENT'] ) .
    ...
    /**
     * Récupère l'IP d'un utilisateur pour requêter l'API d'ISIDORE
     *
     * @return string ip d'un utilisateur
     */
    private function isidore_get_user_ip() {
        $ip = '';
        if ( array_key_exists('HTTP_CLIENT_IP', $_SERVER) and $_SERVER['HTTP_CLIENT_IP'] != null ) {
            $ip = $_SERVER['HTTP_CLIENT_IP'];
        } else if ( array_key_exists('HTTP_X_FORWARDED_FOR', $_SERVER) and $_SERVER['HTTP_X_FORWARDED_FOR'] != null ) {
     ...
     */
    private function isidore_get_lang() {
        $locale = get_locale();
	...
	static function isidore_suggestions_autocomplete() {
		...
		$url			=	'http://api.rechercheisidore.fr/repository/suggest?afs:query=' . $term;
			...
			if ( isset( $afs_suggestions['feed-subject'] ) ) {
				...
				if ( count( $subjects ) > 0 ) {
					foreach ( $subjects as $subject ) {
						...
						$suggestion['link']		= '//www.rechercheisidore.fr/search?q=' . urlencode( $subject );
			...
			if ( isset( $afs_suggestions['feed-creator'] ) ) {
				...
				if ( count( $creators ) > 0 ) {
					foreach ( $creators as $creator ) {
						...
						$suggestion['link']		= '//www.rechercheisidore.fr/search?q=' . urlencode( $creator );
    ...
    static function load_list_isidore_suggestions() {
		...
		if( preg_match( '#^http://api\.rechercheisidore\.fr/repository/search#', $query ) and ctype_digit( strval( $limit ) ) and ctype_digit( strval( $page ) ) ) {
				...
				if ( $load ) {
					...
					if ( $nodes->length > 0 ) {
						foreach ( $nodes as $node ) {
							...
							$ul .= '<li><a href="//www.rechercheisidore.fr/search/resource/?uri=' . $uri . '" target="_blank">' . $title . '</a></li>';
						}
